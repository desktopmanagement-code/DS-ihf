...
New-Item "config\ds_access_tokenJWT.txt" -Force
...
$outputFile = [System.IO.Path]::Combine($PSScriptRoot, "..\config\ds_access_tokenJWT.txt") | Resolve-Path
#$outputFileALL = [System.IO.Path]::Combine($PSScriptRoot, "..\config\ds_access_tokenJWTall.txt") | Resolve-Path
...
if (!(test-path ".\config\ConfirmationProd.txt")){

DSConfirmation
New-Item ".\config\ConfirmationProd.txt" -Force
}
...
try {
    ...
    write-output ($tokenResponse) > $outputFileall
